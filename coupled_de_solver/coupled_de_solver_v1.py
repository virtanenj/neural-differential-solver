...
def coupled_diff_eq_1_squarred(x):
    return coupled_diff_eq_1(x)**2


def coupled_diff_eq_2_squarred(x):
...
def my_loss_fn(x):
    def loss(y_true, y_pred):
        # L = (1/i_max)(F_1^2 + F_2^2) + (f_1(0) - 0)^2 + (f_2(0) - 2)^2
...
# # Cleare the ckeckpoints folder before
...
for i in range(steps):
    ...
    file_path_i = 'checkpoints/checkpoint_' + str(i)
...
# Plot oc the correct solution and solutions of every iteration
